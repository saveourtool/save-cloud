...
import com.saveourtool.save.cosv.security.OrganizationPermissionEvaluator
import com.saveourtool.save.cosv.service.GitService
...
internal class OrganizationController(
    ...
    @Parameters(
        ...
        Parameter(
            ...
            description = "amount of organizations that should be returned, default: 5",
            required = false
        ),
    )
    ...
    fun getOrganizationNamesByPrefix(
        ...
        @RequestParam(required = false, defaultValue = "5") pageSize: Int,
    ): Mono<OrganizationDtoList> = getFilteredOrganizationDtoList(
        OrganizationFilter(prefix), pageSize)
        .collectList()
    ...
    @Parameters(
        ...
        Parameter(
            name = "isAbleToToBulkUpload",
            `in` = ParameterIn.QUERY,
            description = "new flag for bulk upload files ability",
            required = true
        ),
    )
    ...
    fun saveOrganization(
        ...
        when (it.second) {
            OrganizationSaveStatus.CONFLICT -> throw ResponseStatusException(
                HttpStatus.CONFLICT,
                OrganizationSaveStatus.CONFLICT.message
            )
            // Small hack here just not to affect frontend rendering: we return CONFLICT status
            OrganizationSaveStatus.INVALID_NAME -> throw ResponseStatusException(
                HttpStatus.CONFLICT,
                OrganizationSaveStatus.INVALID_NAME.message
            )
            else -> it
        }
    }
