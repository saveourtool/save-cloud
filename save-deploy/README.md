...
basic HTTP security. To access this data from prometheus it should have access to these credentials from docker secrets.
Configuration file [prometheus.yml](./prometheus.yml) is configured to use username `prometheus` and password from standard
path for docker swarm secrets.
