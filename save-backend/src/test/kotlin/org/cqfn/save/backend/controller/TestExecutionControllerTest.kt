...
import org.cqfn.save.backend.utils.millisToLocalDateTime
...
class TestExecutionControllerTest {
    ...
    fun `should save TestExecutionDto into the DB`() {
        ...
        assertTrue(tests.any { it.startTime == testExecutionDtoFirst.startTimeSeconds!!.millisToLocalDateTime().withNano(0) })
        assertTrue(tests.any { it.endTime == testExecutionDtoFirst.endTimeSeconds!!.millisToLocalDateTime().withNano(0) })
