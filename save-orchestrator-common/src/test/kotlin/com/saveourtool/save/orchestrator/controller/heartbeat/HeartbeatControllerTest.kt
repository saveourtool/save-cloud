...
class HeartbeatControllerTest {
    ...
    fun checkAcceptingHeartbeat() {
        val heartBeatBusy = Heartbeat(
            agentInfo = "test".toAgentInfo(),
            state = AgentState.BUSY,
            executionProgress = ExecutionProgress(0, -1L),
    ...
    fun `should not shutdown any agents when not all of them are IDLE`() {
        testHeartbeat(
            ...
            heartbeats = listOf(Heartbeat(
                agentInfo = "test-1".toAgentInfo(),
                state = AgentState.IDLE,
                executionProgress = ExecutionProgress(100, -1L),
                timestamp = Clock.System.now() + 30.seconds
            )),
        ...
        testHeartbeat(
            ...
            heartbeats = listOf(
                Heartbeat(
                    ...
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 1.seconds
                ),
                Heartbeat(
                    agentInfo = "test-1".toAgentInfo(),
                    state = AgentState.IDLE,
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 2.seconds
                )
            ),
    ...
    fun `should shutdown agent, which don't sent heartbeat for some time`() {
        ...
        testHeartbeat(
            ...
            heartbeats = listOf(
                ...
                Heartbeat(
                    agentInfo = "test-2".toAgentInfo(),
                    state = AgentState.BUSY,
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 3.seconds
                ),
                ...
                Heartbeat(
                    ...
                    state = AgentState.BUSY,
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 4.seconds
                ),
                Heartbeat(
                    agentInfo = "test-1".toAgentInfo(),
                    state = AgentState.BUSY,
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 5.seconds
                ),
                Heartbeat(
                    agentInfo = "test-1".toAgentInfo(),
                    state = AgentState.BUSY,
                    executionProgress = ExecutionProgress(0, -1L),
                    timestamp = currTime + 10.seconds
                ),
            ),
    ...
    fun `should mark test executions as failed if agent returned only part of results`() {
        ...
        whenever(orchestratorAgentService.markAllTestExecutionsOfAgentsAsFailed(any(), any()))
        ...
        testHeartbeat(
            ...
            verify(orchestratorAgentService).markAllTestExecutionsOfAgentsAsFailed(any(), any())
        }
    }
