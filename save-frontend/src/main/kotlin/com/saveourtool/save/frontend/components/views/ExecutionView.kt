...
class ExecutionView : AbstractView<ExecutionProps, ExecutionState>(false) {
    ...
    override fun ChildrenBuilder.render() {
        ...
        testExecutionsTable {
            ...
            getData = { page, size ->
                post(
                    url = "$apiUrl/testExecutions?executionId=${props.executionId}&page=$page&size=$size&checkDebugInfo=true",
            ...
            getPageCount = { pageSize ->
                val count: Int = post(
                    url = "$apiUrl/testExecutions?executionId=${props.executionId}&page=1&size=$pageSize&checkDebugInfo=true",
                    headers = Headers().apply {
                        set("Accept", "application/json")
                        set("Content-Type", "application/json")
                    },
